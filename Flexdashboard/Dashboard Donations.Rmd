...
reactiveDonations <- reactive({
 ...
 dataset_donations_base %>% 
  group_by_at(vars(MY, input$levelsid)) %>%
    ...
    select(Donation.Month = donation.month, Value = value, Donation.Year = donation.year, everything())
...
selectInput(
  inputId = "graphid2",
  label = "Select the Graph to Display",
  choices =  c("New Donors" = "sum.new.donors", "Total Donors" = "donors.per.month"),
  multiple = FALSE
)
...
## Create reactive dataset that will depend on date, nominal, source.group and source 

reactiveTotalDonors <- reactive({
dataset_donations_base %>%
  group_by_at(vars(MY, input$levelsid2)) %>%
  distinct(donor.no, .keep_all = TRUE) %>%
  mutate(donors.per.month = n()) %>%
  ungroup() %>% 
    select(Donation.Month = donation.month, Donation.Year = donation.year, everything())

})


reactiveTotalDonorsFiltered<- reactive({
  if (input$levelsid2 == "nominal" & input$graphid2 == "donors.per.month") {
  reactiveTotalDonors() %>%
      filter(
  ...
  } else if (input$levelsid2 == "source.group" & input$graphid2 == "donors.per.month") {
    reactiveTotalDonors() %>%
      filter(
      donation.date >= input$dateid2[1] &
      donation.date <= input$dateid2[2],
  ...
  } else if (input$levelsid2 == "source" & input$graphid2 == "donors.per.month") {
    reactiveTotalDonors() %>%
      filter(
      donation.date >= input$dateid2[1] &
      donation.date <= input$dateid2[2],
...
renderPlotly({
  req(input$actionid2)
  if (input$graphid2 == "donors.per.month") {
  ggplotly(
  reactiveTotalDonorsFiltered() %>%
  ggplot(
  aes(
  Donation.Month,
  donors.per.month,
...
# } else if (input$graphid2 == "sum.new.donors") {
  # ggplotly(
  # reactiveNewDonorsFiltered() %>%
  # ggplot(
  # aes(
  # donation.month,
  # sum.new.donors,
