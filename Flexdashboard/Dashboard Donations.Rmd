...
observeEvent(input$nominalid, {
  ...
  updateSelectInput(session, "source.groupid", choices = c("", filtered_donations1))
...
reactiveDonations<- reactive({
  ...
  } else if (!input$nominalid == "" & is.null(input$source.group) & is.null(input$source)){ # DONE
...
} else if (!(input$nominalid == "") & !(input$source.group == "") & !(input$source == "")){
