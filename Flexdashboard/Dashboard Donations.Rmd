...
reactive_viz_donations <- reactive({
  ...
  ungroup() %>% 
  distinct(MY, input$levelsid, .keep_all = TRUE) # In shiny gere goes input$Nominal, source...
...
reactive_viz_general <- reactive({
  ...
  left_join(reactive_viz_newdonors(), by = c("MY", input$levelsid)) %>% 
  left_join(reactive_viz_totaldonors(), by = c("MY", input$levelsid)) %>%  
  select(nominal,
         donation.date,
         source.group,
         source, 
